defmodule IotaService.Integration.NotarizationTest do
  ...
  describe "hashing" do
    test "produces consistent SHA-256 hashes" do
      hash_a = IotaService.hash("hello")
      hash_b = IotaService.hash("hello")
    ...
    test "produces known SHA-256 hash for 'hello'" do
      assert IotaService.hash("hello") ==
               "2cf24dba5fb0a30e26e83b2ac5b9e29e1b161e5c1fa7425e73043362938b9824"
    end

    test "different inputs produce different hashes" do
      hash_a = IotaService.hash("document A")
      hash_b = IotaService.hash("document B")

      assert hash_a != hash_b
    end

    test "handles binary data" do
      hash = IotaService.hash(<<0, 1, 2, 3, 255>>)
  ...
  describe "payload creation" do
    test "creates a valid notarization payload" do
      assert {:ok, payload} = IotaService.notarize("important document")
    ...
    test "payload hash matches direct hash of data" do
      ...
      expected_hash = IotaService.hash(data)

      assert {:ok, payload} = IotaService.notarize(data)
      assert payload["data_hash"] == expected_hash
    end

    test "creates payload with custom tag" do
      assert {:ok, payload} = IotaService.notarize("data", "custom_tag")
      assert is_map(payload)
    end

    test "different data produces different payloads" do
      {:ok, payload_a} = IotaService.notarize("document A")
      {:ok, payload_b} = IotaService.notarize("document B")
  ...
  describe "payload verification" do
    test "verifies a valid hex string" do
      assert IotaService.Notarization.Server.valid_hex?("deadbeef")
      assert IotaService.Notarization.Server.valid_hex?("0123456789abcdef")
    end

    test "rejects invalid hex strings" do
      refute IotaService.Notarization.Server.valid_hex?("not_hex!")
      refute IotaService.Notarization.Server.valid_hex?("ZZZZ")
    end
  end

  describe "notarization queue" do
    test "enqueues a job and retrieves status" do
      assert {:ok, ref} = IotaService.enqueue_notarization("queued data", "test_tag")
      assert is_reference(ref)

      assert {:ok, job} = IotaService.Notarization.Queue.status(ref)
    ...
    test "returns :not_found for unknown job ref" do
      ...
      assert :not_found = IotaService.Notarization.Queue.status(unknown_ref)
    end

    test "reports queue stats" do
      stats = IotaService.queue_stats()
  ...
  describe "full notarization lifecycle" do
    test "hash → notarize → verify payload integrity" do
      ...
      hash = IotaService.hash(data)
      assert String.length(hash) == 64

      # Step 2: Create notarization payload
      assert {:ok, payload} = IotaService.notarize(data, "legal_doc")
      assert payload["data_hash"] == hash

      # Step 3: Enqueue for batch processing
      assert {:ok, ref} = IotaService.enqueue_notarization(data, "legal_doc")
      assert {:ok, job} = IotaService.Notarization.Queue.status(ref)
      assert job.status == :pending
    end
  end
